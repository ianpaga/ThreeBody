...
#SBATCH -o bt_%J         # create outfile
#SBATCH -e bt_%J         # create errorfile
#SBATCH --time=1-00:00:00        # specify the requested wall-time
#SBATCH --partition=dark         # specify the partition to run on
#SBATCH --nodes=5                # number of nodes allocated for this job
#SBATCH --ntasks-per-node=20     # number of MPI ranks per node, 20 for astro_short and astro_long
#SBATCH --cpus-per-task=1        # number of OpenMP threads per MPI rank
##SBATCH --exclude=<node list>   # avoid nodes (e.g. --exclude=node786)

#SBATCH --mail-user=ianpaga@slac.stanford.com # please put your own email, I don't want your error messages
